...
- **不同系统要用不同编译产物**（依赖的 libc、库版本不同），所以 `focal` / `jammy` / `noble` 各有一套索引。
- **`.deb` 文件本身集中在 `pool/`**，索引用路径指过去，避免同一文件复制很多份。
- 本仓库为避免「同名同版本冲突」，会把版本写成 `1.0.0~jammy1`、`1.0.0~focal1` 这种带套件后缀的形式。
...
```text
deb [signed-by=/usr/share/keyrings/linux-apt-repo.gpg] https://YOUR_URL jammy main
```

若暂时不做签名，也可以写 `deb [trusted=yes] ...`（不推荐，等于不验章）。
...
```mermaid
flowchart LR
  A[source/ 软件源码] --> D[Docker 构建环境<br/>focal/jammy/noble]
...
SUITES="focal jammy noble"    # 要支持的 Ubuntu
...
把下面的 URL 换成你的 Pages 地址：

```bash
curl -fsSL https://YOUR_PAGES_URL/add-apt-source.sh \
  | sudo bash -s -- https://YOUR_PAGES_URL
```

或手动：

```bash
curl -fsSL https://YOUR_PAGES_URL/repo-key.gpg \
  | sudo gpg --dearmor -o /usr/share/keyrings/linux-apt-repo.gpg

echo "deb [signed-by=/usr/share/keyrings/linux-apt-repo.gpg] https://YOUR_PAGES_URL $(lsb_release -cs) main" \
...
`$(lsb_release -cs)` 会自动变成 `jammy` / `noble` 等，从而用对那一套索引和包。
...
推到 `main`/`master` 后自动：生成 `dists/`/`pool/` → 部署 GitHub Pages。  
仓库 **Settings → Pages → Source** 选 **GitHub Actions**。  
推荐 Secrets：`APT_REPO_GPG_PRIVATE_KEY`。源码就绪后可在 Actions 手动运行并勾选构建 `.deb`。
